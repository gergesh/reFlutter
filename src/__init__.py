...
def networkLib():
    ...
    if len(libios[1]) != 0:
        try:
            urlretrieve(
                "https://github.com/ptswarm/reFlutter/releases/download/ios-"
                + libios[1]
                + "/Flutter",
                "Flutter",
            )
    ...
    if len(libAppX64[1]) != 0:
        ...
        try:
            urlretrieve(
                "https://github.com/gergesh/reFlutter/releases/download/android-"
                + libAppX64[1]
                + "/libflutter_x64.so",
                "libflutter_x64.so",
            )
...
def convertIPFix():
    ...
    for i in intoct:
        if (
            ...
            len(i) != 3 and int(i) > 7 and int(i) < 64 and len(".".join(intoct)) < 15
        ):  # 8-63
            intoct[intoct.index(i)] = str(oct(int(i))).replace("o", "")
...
def patchSource(hashS, ver):
    replaceFileText(
        "src/third_party/dart/runtime/vm/dart.cc", "FLAG_print_class_table)", "true)"
    )
    if ver > 27:
        replaceFileText(
